...
function CurrentTile({ tileSets }: CurrentTileProps) {
  ...
  return (
    <div className='flex flex-col bg-gray-800 h-24 w-24 rounded-full p-3 items-center'>
      <div className=''>
        {selectedTileSprite ? (
          <AtlasRegion
...
export default function Dock() {
  ...
  return (
    <div className='flex flex-row justify-around w-2/3 mb-1 m-auto p-2 absolute left-0 right-0 bottom-0 items-end'>
      <style>
        {Object.entries(tileSets).map(([id, tileSet]) => ` .tileset-bg-${id} { background-image: url('${tileSet.image}');} `)}
      </style>
      <CurrentTile tileSets={tileSets} />
      <div className='w-2/3 flex-grow ml-2 p-2 rounded-sm bg-gray-900 flex flex-row justify-between items-center'>
